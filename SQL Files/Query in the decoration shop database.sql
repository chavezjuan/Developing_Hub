...
/* Number of Sales */
SELECT product, COUNT(*) AS each_sale FROM sales GROUP by product;

/*Total Income for Each Product*/
SELECT product, SUM(price) AS Income FROM sales GROUP by product;

SELECT * FROM sales WHERE country="United states" AND payment_type="Visa";

SELECT payment_type, SUM(price) AS payments_card FROM sales GROUP BY payment_type HAVING payments_card>50000;

/*Selection of potential clients*/

SELECT name, price,
 CASE
    WHEN price = ROUND(1200) THEN "low profile client"
    WHEN price = ROUND(3600) THEN "potential client"
    WHEN price = ROUND(7500) THEN "special client"
 END as "type_of_client"
FROM sales 
/*Show only the special clients*/
WHERE type_of_client LIKE"special client";
